...
/*   Updated: 2024/02/09 01:21:06 by greus-ro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	node = (*stack_b);
	num_rbs = 0;
	while((size_t)node->content > chunk.min + chunk_size / 2 && node->content > value)
	{
		ft_execute_action(stack_a, stack_b, PRIMITIVE_RB);
		num_rbs++;
		node = node->next;	
...
/*
static void	ft_sort_chunk(t_stack **stack_a, t_stack **stack_b, t_chunk chunk)
{
	t_stack *node;
	size_t num_push;

	node = *stack_b;
	while(node != NULL && node->next != NULL && node->content < node->next->content)
	{
			...
			t_chunk chunk)
{
	...
	while (numbers_found < chunk.max - chunk.min)
	{
		...
		*/
		//ft_stack_debug(*stack_b);
		//ft_stackb_max_to_top(stack_b);
		//ft_sort_chunk(stack_a, stack_b, chunk);
		numbers_found++;
	}
	/*
	printf("\t\tFINAL\n");
